class Boat < ApplicationRecord
  ...
  def check_availability(date, start, finish)
    ...
    conflicting_events.each do |e|
      ...
      if e.is_on?
        message = "#{e.team.name} crew is using the #{name} from #{e.start_time.strftime('%H:%M')} to #{e.end_time.strftime('%H:%M')} (boat full)"
      else
        message = "#{e.team.name} crew is possibly using the #{name} from #{e.start_time.strftime('%H:%M')} to #{e.end_time.strftime('%H:%M')} (boat not full)"
      end
      return message
    end
    nil
